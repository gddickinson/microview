...
class LightsheetViewer(QMainWindow):
    ...
    def updateUIForNewData(self):
        if self.data is not None:
            ...
            if self.ui_manager.channelControlsLayout is not None:
                for i in reversed(range(self.ui_manager.channelControlsLayout.count())):
                    item = self.ui_manager.channelControlsLayout.itemAt(i)
                    if item.widget():
                        item.widget().setParent(None)
                self.ui_manager.channel_controls.clear()
            ...
            for i in range(num_channels):
                ...
                self.ui_manager.channel_controls.append((visibilityCheck, opacitySlider))

                channelWidget = QWidget()
                channelWidget.setLayout(channelLayout)
                self.ui_manager.channelControlsLayout.addWidget(channelWidget)
