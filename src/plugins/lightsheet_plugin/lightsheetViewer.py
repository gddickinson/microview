...
class LightsheetViewer(QMainWindow):
    ...
    def runBiologicalSimulation(self, params):
        try:
            ...
            self.updateAllViews()  # Use the new method here
    ...
    def updateAllViews(self):
        if self.data is not None:
            ...
            self.logger.info("All views updated with new data")
    ...
    def updateRawDataViewer(self):
        ...
        if self.data is not None:
            # Ensure the data is in the correct format (t, c, z, y, x)
            if self.data.ndim == 5:
                raw_data = self.data
            elif self.data.ndim == 4:
                raw_data = self.data.reshape(1, *self.data.shape)
            elif self.data.ndim == 3:
                raw_data = self.data.reshape(1, 1, *self.data.shape)
            else:
                self.logger.error(f"Unexpected data shape: {self.data.shape}")
                return

            properties = {
                'num_z_slices': raw_data.shape[2],
            ...
            self.raw_data_viewer.setData(raw_data, properties)
