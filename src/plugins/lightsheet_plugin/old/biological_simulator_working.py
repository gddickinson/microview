...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QVBoxLayout, QHBoxLayout, QWidget,
                             ...
                             QTableWidgetItem, QDialog, QGridLayout, QTabWidget, QTextEdit, QAction, QFormLayout, QGroupBox)
...
#from sisImporter import SISImporter as SISReader
...
#         if file_path.endswith('.sis'):
#             return SISImporter(file_path)
    ...
    def generate_mitochondria(self, num_mitochondria: int = 50, size_range: Tuple[int, int] = (3, 8)):
        try:
            ...
            for _ in range(num_mitochondria):
                ...
                if attempts == 100:
                    self.logger.warning(f"Failed to place mitochondrion after 100 attempts")
...
class BiologicalSimulationWidget(QWidget):
    ...
    def initUI(self):
        ...
        self.create_organelles_tab()  # New tab for mitochondria and cytoskeleton
    ...
    def create_structure_tab(self):
        ...
        # Cellular Structures Combo (keep this for backwards compatibility)
        ...
        # Individual structure checkboxes
        ...
        main_layout.addWidget(self.cell_membrane_checkbox)
        main_layout.addWidget(self.nucleus_checkbox)
        main_layout.addWidget(self.er_checkbox)
        main_layout.addWidget(self.mitochondria_checkbox)
        main_layout.addWidget(self.cytoskeleton_checkbox)
    ...
    def create_organelles_tab(self):
        ...
        # Mitochondria options
        mito_group = QGroupBox("Mitochondria")
        mito_layout = QFormLayout()
        ...
        mito_size_layout = QHBoxLayout()
        mito_size_layout.addWidget(self.mito_size_min)
        mito_size_layout.addWidget(QLabel("-"))
        mito_size_layout.addWidget(self.mito_size_max)
        mito_layout.addRow("Size Range:", mito_size_layout)
        mito_group.setLayout(mito_layout)
        layout.addWidget(mito_group)

        # Cytoskeleton options
    ...
    def requestSimulation(self):
        params = {
            ...
            'cellular_structure': self.structure_combo.currentText(),  # Keep this for backwards compatibility
...
class LightsheetViewer(QMainWindow):
    ...
    def runBiologicalSimulation(self, params):
        try:
            ...
            # Generate cell shape and membrane
            ...
            if cellular_structures.get('mitochondria', False):
                self.logger.info("Attempting to generate mitochondria")
                if 'mitochondria' not in params:
                    self.logger.error("Mitochondria parameters not found in params")
                    raise ValueError("Mitochondria parameters not found")

                mito_count = params['mitochondria'].get('count', 50)
                mito_size_range = params['mitochondria'].get('size_range', (3, 8))
                self.logger.info(f"Mitochondria parameters: count={mito_count}, size_range={mito_size_range}")
                ...
                if self.data is None:
                    self.data = mitochondria_timeseries
                else:
                    self.data = np.concatenate((self.data, mitochondria_timeseries), axis=1)
                self.logger.info(f"Added mitochondria data. New data shape: {self.data.shape}")

            # Generate cytoskeleton
            if cellular_structures.get('cytoskeleton', False):
                self.logger.info("Attempting to generate cytoskeleton")
                if 'cytoskeleton' not in params:
                    self.logger.error("Cytoskeleton parameters not found in params")
                    raise ValueError("Cytoskeleton parameters not found")
                ...
                self.logger.info(f"Cytoskeleton parameters: actin_density={actin_density}, microtubule_density={microtubule_density}")
                ...
                if self.data is None:
                    self.data = np.concatenate((actin_timeseries, microtubules_timeseries), axis=1)
                else:
                    self.data = np.concatenate((self.data, actin_timeseries, microtubules_timeseries), axis=1)
                self.logger.info(f"Added cytoskeleton data. New data shape: {self.data.shape}")
            ...
            # Simulate active transport if enabled
            if params['active_transport']['enabled']:
                ...
                transport_timeseries = transport_data[:, np.newaxis, :, :, :]
                if self.data is None:
                    self.data = transport_timeseries
                else:
                    self.data = np.concatenate((self.data, transport_timeseries), axis=1)
                self.logger.info(f"Added active transport data. New data shape: {self.data.shape}")
            ...
            if params['calcium_signal']['type'] != 'None':
                ...
                calcium_timeseries = calcium_signal[:, np.newaxis, :, :, :]
                if self.data is None:
                    self.data = calcium_timeseries
                else:
                    self.data = np.concatenate((self.data, calcium_timeseries), axis=1)
                self.logger.info(f"Added calcium signal data. New data shape: {self.data.shape}")
    ...
    def create3DVisualization(self):
        try:
            ...
            # Define more colors to accommodate all channels
            channel_colors = [
                (1, 0, 0, 1),    # Red
                (0, 1, 0, 1),    # Green
                (0, 0, 1, 1),    # Blue
            ...
            for c in range(num_channels):
                if c < len(self.channelControls) and self.channelControls[c][0].isChecked():
                    ...
                    if len(pos) > 0:
                        # Use modulo to cycle through colors if there are more channels than colors
                        color = channel_colors[c % len(channel_colors)]
