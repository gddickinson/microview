...
class TestBiologicalDataGenerator(unittest.TestCase):
    ...
    def test_generate_volume(self):
        ...
        self.assertTrue(np.any(volume == 0.5))  # ER
        self.assertTrue(np.any(volume == 0.7))  # mitochondria
    ...
    def test_generate_time_series(self):
        ...
        # Check that volumes are not identical (due to protein diffusion)
        self.assertFalse(np.allclose(time_series[0], time_series[-1]))
