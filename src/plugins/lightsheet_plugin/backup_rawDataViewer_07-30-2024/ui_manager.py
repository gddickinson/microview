...
class UIManager:
    ...
    def create_data_generation_widget(self):
        ...
        self.parent.structuredDataCheck.stateChanged.connect(self.toggleStructuredDataControls)
        layout.addWidget(self.parent.structuredDataCheck)

        # Structured data controls
        self.structuredDataWidget = QWidget()
        structuredLayout = QVBoxLayout(self.structuredDataWidget)
        self.channelRangeWidgets = []

        for i in range(2):  # Assuming 2 channels, adjust as needed
            channelWidget = QWidget()
            channelLayout = QHBoxLayout(channelWidget)
            channelLayout.addWidget(QLabel(f"Channel {i+1} Range:"))
            for axis in ['X', 'Y', 'Z']:
                minSpinBox = QSpinBox()
                maxSpinBox = QSpinBox()
                minSpinBox.setRange(0, 100)
                maxSpinBox.setRange(0, 100)
                channelLayout.addWidget(QLabel(f"{axis} Min:"))
                channelLayout.addWidget(minSpinBox)
                channelLayout.addWidget(QLabel(f"{axis} Max:"))
                channelLayout.addWidget(maxSpinBox)
            structuredLayout.addWidget(channelWidget)
            self.channelRangeWidgets.append((minSpinBox, maxSpinBox, minSpinBox, maxSpinBox, minSpinBox, maxSpinBox))

        layout.addWidget(self.structuredDataWidget)
        self.structuredDataWidget.setVisible(False)
    ...
    def toggleStructuredDataControls(self, state):
        self.structuredDataWidget.setVisible(state == Qt.Checked)
