class BinaryOperations:
    ...
    def threshold(self):
        if self.parent.window_manager.current_window:
            ...
            self.parent.window_manager.current_window.setImage(binary)

    def erode(self):
        if self.parent.window_manager.current_window:
            image = self.parent.window_manager.current_window.image
            eroded = self.parent.image_processor.erode(image)
            self.parent.window_manager.current_window.setImage(eroded)

    def dilate(self):
        if self.parent.window_manager.current_window:
            image = self.parent.window_manager.current_window.image
            dilated = self.parent.image_processor.dilate(image)
            self.parent.window_manager.current_window.setImage(dilated)
